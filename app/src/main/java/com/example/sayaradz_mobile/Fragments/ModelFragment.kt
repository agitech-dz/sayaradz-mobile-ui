...
class ModelFragment : Fragment() {
    ...
    private fun loadData(){
        ...
        if (Utilities.hasNetwork(context!!)){
            compositeDisposable?.add(restService.getModels(1,50)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(this::handleModelsResponse,this::handleError))
    ...
    private fun handleModelsResponse(modelList: List<Model>){
    ...
    private fun setUpNotificationRecyclerView() {
        ...
        adapter = ModelListAdapter(modelList, context!!)
